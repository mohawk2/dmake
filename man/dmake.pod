...
All  attributes are user settable and except for .UPDATEALL and .MKSARGS
...
The  inferred names of targets :i modifier returns the actual filename
...
If aprog and bprog are targets or prerequisites and they  are  bound  to
...
No special treatment of builtin commands for group  recipes  is
implemented  even  though  the E<lt>empty recipeE<gt> will most probably
...
definition without recipe lines, it would normally issue a "Don't know
...
probably would complain about this.
...
operating systems or built with other compilers.
...
`-'.  The macro is internally defined and is not user settable.   The MSDOS
...
If all expand to non-empty strings the macro returns
...
Echoes the value of list.  list is not expanded.
...
expands macroterm and returns the string "t" if the
...
expands  each  macroterm  in  turn  and  returns
the empty string if each term expands to the empty
string;  otherwise, it returns the string "t".
...
and their values are no longer available.  Activation occurs after all
...
systems, long member file names.  A small example best illustrates this.
  ...
  mylib.a .LIBRARY : mem1.o mem2.o mem3.o ; rules for making library...
...
target  that does  not  possess the .NOSTATE attribute will have a record
...
The Sun Microsystems Make construct
...
fifth command it will run immediately upon a child process becoming free.
...
of a group recipe will be mapped to the ".INCLUDE"
